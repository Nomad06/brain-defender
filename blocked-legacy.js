async function pickPhrase() {
  ...
  (async () => {
    ...
    // Обработчики временных исключений
    const allow15min = document.getElementById('allow15min');
    const allow30min = document.getElementById('allow30min');
    const allow1hour = document.getElementById('allow1hour');
    const allow2hours = document.getElementById('allow2hours');
    const tempAllowStatus = document.getElementById('tempAllowStatus');
    
    async function requestTempAccess(minutes) {
      if (!window.tempWhitelist || !target.host) {
        return;
      }
      
      try {
        const success = await window.tempWhitelist.addTempWhitelist(target.host, minutes);
        if (success) {
          if (tempAllowStatus) {
            tempAllowStatus.style.display = 'block';
            tempAllowStatus.textContent = `Доступ разрешен на ${minutes} минут. Обновите страницу.`;
            tempAllowStatus.style.color = 'var(--accent)';
          }
          ...
          // Через небольшую задержку пытаемся перезагрузить страницу
          setTimeout(() => {
            if (target.full) {
          ...
          if (tempAllowStatus) {
            tempAllowStatus.style.display = 'block';
            tempAllowStatus.textContent = 'Ошибка при добавлении временного исключения';
            tempAllowStatus.style.color = 'var(--danger)';
          }
        }
      } catch (err) {
        console.error('Error requesting temp access:', err);
        if (tempAllowStatus) {
          tempAllowStatus.style.display = 'block';
          tempAllowStatus.textContent = 'Ошибка: ' + err.message;
          tempAllowStatus.style.color = 'var(--danger)';
        }
      }
    }
    ...
    if (allow15min) {
      allow15min.addEventListener('click', () => requestTempAccess(15));
    }
    if (allow30min) {
      allow30min.addEventListener('click', () => requestTempAccess(30));
    }
    if (allow1hour) {
      allow1hour.addEventListener('click', () => requestTempAccess(60));
    }
    if (allow2hours) {
      allow2hours.addEventListener('click', () => requestTempAccess(120));
    }
    
    // Проверяем, есть ли уже активное исключение
    if (window.tempWhitelist && target.host) {
      const remaining = await window.tempWhitelist.getRemainingTime(target.host);
      if (remaining !== null && remaining > 0) {
        if (tempAllowStatus) {
          tempAllowStatus.style.display = 'block';
          tempAllowStatus.textContent = `У вас уже есть временный доступ на ${remaining} минут`;
          tempAllowStatus.style.color = 'var(--accent)';
        }
      }
    }
...
(function() {
  ...
  function initEyeCanvas() {
    ...
    eyeCanvasCtx.setTransform(1,0,0,1,0,0);
